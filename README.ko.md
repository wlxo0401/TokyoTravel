...
- **여행 중 — 참고(읽기 전용)**: 이때는 계획을 고치지 않습니다. 미리 준비해 둔 정보를
  **그 자리에서 확인하고 빠르게 결정**하는 용도입니다.
  - 날짜 탭으로 오늘 동선을 한눈에
  - 도착 → 지역 → 짐 → 체크인으로 이어지는 그날 동선을 순서대로 (빡빡한 시간표 아님)
  - 버튼 하나로 지도 길찾기 (한국 구간은 네이버, 일본 구간은 구글)
  - 지점 사이 이동편과 코인락커·짐 같은 메모도 그 자리에서
...
> [!WARNING]
> **예약번호·개인정보는 직접 입력하지 마세요.** `data/trip.js`는 저장소에 커밋되어
> GitHub Pages로 공개 배포되며, 나중에 지워도 git 기록에 남습니다. 예약번호, 여권·신분증
> 번호, 전화번호, 집 상세 주소, 도어락 비밀번호 같은 정보는 넣지 말고 일행과는 비공개
> 채널로 공유하세요. 장소 이름·지역·지도 좌표 정도는 괜찮습니다.
